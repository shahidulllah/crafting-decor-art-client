...
const Card = ({ item }) => {
    ...
    return (
        <div className="rounded-2xl shadow-2xl border-2 ">
            <img src={item.imageUrl} alt="" className="object-cover object-center w-full rounded-t-md h-96" />
            <div className="flex flex-col justify-between p-6 space-y-8">
               ...
               <div>
               <Link to={`/viewDetails/${item._id}`}> <button type="button" className="flex btn  items-center justify-center w-full p-3 font-extrabold tracking-wide rounded-md  "><span className='text-blue-400'><TbListDetails className='text-xl' /></span> View Details</button></Link>
               </div>
            </div>
        </div>
    );
